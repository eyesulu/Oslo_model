...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 15 14:10:21 2023
...
for i in range (20000):
    a.start()
    heights_a.append(a._sites[0]._site_height)
...
for i in range (20000):
    b.start()
    heights_b.append(b._sites[0]._site_height)
...
for i in range (500):
    BTW_model.start()
    heights_BTW.append(BTW_model._sites[0]._site_height)
    Oslo_model.start()
    heights_Oslo.append(Oslo_model._sites[0]._site_height)
    
plt.plot(test_index,heights_BTW, color = 'orange', label = 'BTW model')
plt.plot(test_index,heights_Oslo, color ='blue', label = 'Oslo model')
plt.legend()
...
for k in range (test_threshold._L):
    threshold_vals.append(test_threshold._sites[k]._threshold)
...
for i in range (10000):
    for j in range (test_threshold._L):
        threshold_vals.append(test_threshold._sites[j]._threshold)
...
#The following test demonstrates the animation for L = 4 specifically

test_anim = Oslo (4, [1, 2], [0.5, 0.5])
for i in range (30):
    test_anim.start()
    test_anim.animate()
    pl.pause(0.05)
    print(test_anim.get_heights())
